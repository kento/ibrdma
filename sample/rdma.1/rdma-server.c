...
{  
  ...
  while (1) {
	...
	switch (conn->recv_msg->type)
	  {
	  case MR_INIT:
	    printf("Recieved: MR_INT: Type=%u, buff size=%lu\n", conn->recv_msg->type, conn->recv_msg->data1.buff_size);
	    ...
	    break;
	  case MR_CHUNK:
	    printf("Recived: MR_CHUNK: Type=%d, mr size=%lu\n", conn->recv_msg->type, conn->recv_msg->data1.mr_size);
	  ...
	  case MR_FIN:
	    ...
	    printf("RDMA transfer finished: Tag=%d\n",tag);
	...
	printf("Sent: Sent out: TYPE=%d\n", conn->send_msg->type);
      } else if (wc.opcode == IBV_WC_RDMA_READ) {
	printf("Sent: RDMA: IBV_WC_RDMA_READ ");
      } else {
	die("unknow opecode.");
      }
    }

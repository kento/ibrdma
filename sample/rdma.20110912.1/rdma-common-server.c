...
void on_completion(struct ibv_wc *wc)
{
  ...
  if (wc->opcode & IBV_WC_RECV) {
	...
	send_memr (conn, MR_INIT_ACK, NULL, tfile.size);
      case MR_MSG:
	send_memr (conn, MR_MSG_DONE, NULL, tfile.size);
      case MR_FIN:
	printf("RDMA transfer finished\n");
	send_memr(conn, MR_FIN_ACK, NULL, tfile.size);
      default:
	printf("Unknown TYPE");
      }
    post_receives(conn);
  ...
  if (wc->opcode & IBV_WC_RECV) {
    ...
    if (conn->recv_msg->type == MSG_MR) {
      ...
      memcpy(&conn->peer_mr, &conn->recv_msg->data.mr, sizeof(conn->peer_mr));
      ssize = conn->recv_msg->size;
      /*TODO: free the memory region*/
      printf("Allocate memory: size=%d\n", conn->recv_msg->size);
      char* addr = (char*) malloc(ssize);
      sprintf(addr,"aaaaa%d",ssize);
      build_msg_region(conn, addr, ssize);
